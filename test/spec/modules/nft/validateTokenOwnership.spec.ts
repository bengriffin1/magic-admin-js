...
test('Throws an error if ERC1155 and no token provided', async () => {
  ...
  await expect(sdk.nft.validateTokenOwnership(
    'did:ethr:0x123',
    '0xfoo',
    'ERC1155',
    web3,
  )).rejects.toThrow('ERC1155 requires a tokenId');
});
...
test('Returns an error if ERC721 token is not owned by user', async () => {
    ...
    sdk.users.getMetadataByToken = jest.fn().mockResolvedValue({ email: "test@test.com", publicAddress: "0x610dcb8fd5cf7f544b85290889a456916fbeaba2" });
